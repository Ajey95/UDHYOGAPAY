// Page feature: drives the Payouts_Backup screen and its user interactions.
...
const Payouts: React.FC = () => {
  ...
  const handleSubmit = async () => {
    try {
      const payload = {
        ...
        bookings: formData.bookings.split(',').map(id => id.trim()),
  ...
  const columns: GridColDef[] = [
    { field: '_id', headerName: 'Payout ID', width: 100, renderCell: (params) => params.value.slice(-8) },
    { field: 'workerName', headerName: 'Worker', width: 150, valueGetter: (params) => params.row.worker.userId.name },
    { field: 'profession', headerName: 'Profession', width: 120, valueGetter: (params) => params.row.worker.profession },
    { field: 'totalEarnings', headerName: 'Earnings', width: 120, valueFormatter: (params) => `₹${params.value}` },
    { field: 'commissionPercent', headerName: 'Commission %', width: 120, valueFormatter: (params) => `${params.value}%` },
    { field: 'commissionAmount', headerName: 'Commission', width: 120, valueFormatter: (params) => `₹${params.value}` },
    { field: 'netPayout', headerName: 'Net Payout', width: 120, valueFormatter: (params) => `₹${params.value}` },
    ...
    },
    {
      ...
      renderCell: (params: GridRenderCellParams) => (
        <Box>
          ...
          <IconButton 
            size="small" 
            onClick={() => setConfirmDialog({ open: true, action: 'delete', payoutId: params.row._id })}
            color="error"
          >
          ...
          {params.row.payoutStatus === 'pending' && (
            <IconButton 
              size="small" 
              onClick={() => setConfirmDialog({ open: true, action: 'process', payoutId: params.row._id })}
              color="success"
            >
  ...
  return (
    <>
      ...
      <Box sx={{ p: 3 }}>
              ...
              <Grid container spacing={2}>
                <Grid item xs={6}>
                  <TextField
                    ...
                    onChange={(e) => setFormData({ ...formData, totalEarnings: parseFloat(e.target.value) })}
                    fullWidth
                    required
                  />
                </Grid>
        ...
        >
          <DialogTitle>
            Confirm {confirmDialog.action === 'delete' ? 'Delete' : 'Process'}
          </DialogTitle>
          <DialogContent>
            <Typography>
              Are you sure you want to {confirmDialog.action} this payout?
            </Typography>
          </DialogContent>
          <DialogActions>
            ...
            <Button 
              onClick={confirmDialog.action === 'delete' ? handleDelete : handleProcess} 
