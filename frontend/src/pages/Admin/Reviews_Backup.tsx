// Page feature: drives the Reviews_Backup screen and its user interactions.
...
const Reviews: React.FC = () => {
  ...
  const handleModerate = async (reviewId: string, newStatus: 'approved' | 'rejected') => {
    try {
      await api.patch(`/reviews/${reviewId}/moderate`, { status: newStatus });
  ...
  const columns: GridColDef[] = [
    { field: '_id', headerName: 'Review ID', width: 100, renderCell: (params) => params.value.slice(-8) },
    { field: 'bookingId', headerName: 'Booking ID', width: 100, valueGetter: (params) => params.row.booking._id.slice(-8) },
    { field: 'userName', headerName: 'User', width: 150, valueGetter: (params) => params.row.user.name },
    ...
    },
    {
      ...
      renderCell: (params: GridRenderCellParams) => (
        <Box>
          ...
          <IconButton 
            size="small" 
            onClick={() => setConfirmDialog({ open: true, action: 'delete', reviewId: params.row._id })}
            color="error"
          >
          ...
          {params.row.status === 'pending' && (
            <>
              <Button size="small" onClick={() => handleModerate(params.row._id, 'approved')} color="success">
                Approve
              </Button>
              <Button size="small" onClick={() => handleModerate(params.row._id, 'rejected')} color="error">
                Reject
              </Button>
            </>
          )}
  ...
  return (
    <>
      ...
      <Box sx={{ p: 3 }}>
        ...
        {/* Confirmation Dialog */}
        <Dialog open={confirmDialog.open} onClose={() => setConfirmDialog({ open: false, action: null, reviewId: null })}>
          <DialogTitle>Confirm {confirmDialog.action === 'delete' ? 'Delete' : 'Moderate'}</DialogTitle>
          <DialogContent>
            <Typography>
              Are you sure you want to {confirmDialog.action} this review?
            </Typography>
          </DialogContent>
          <DialogActions>
